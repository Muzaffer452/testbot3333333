...
client.on("ready", () => {
  setInterval(() => {
    let botdurum = client.channels.find(c => c.id === "726546604894453792");
...
client.on("message", async message => {
  ...
  if (
    ...
    if (i === "acik") {
      const embed = new Discord.RichEmbed()
        .setColor("BLUE")
        .setDescription(`:wave: **Aleyküm Selam, Hoşgeldin Dostum! ^-^**`);
...
client.on("guildMemberAdd", async member => {
  ...
  if (rol == member.guild.memberCount) {
    const embed = new Discord.RichEmbed()
      .setColor("BLUE")
      .setDescription(
        `<a:kutlama:703842025946218496> Tebrikler! başarılı bir şekilde ${rol} kişi olduk!`
      )
      .setFooter(client.user.username, client.user.avatarURL);
    client.channels.get(kanal).send(embed);
...
client.on("guildCreate", async guild => {
  ...
  client.channels.get(`726546604894453792`).send(embed);
});
...
client.on("message", async message => {
  const ms = require("parse-ms");
  let dogrulama = await db.fetch(
    `sahiponay_${message.author.id}_${message.guild.id}`
  );
  ...
  if (dogrulama !== null && gun - (Date.now() - dogrulama) > 0) {
    ...
    if (message.author.id === ayarlar.sahip) {
      ...
      message.channel
        .send("**Hizaya Geçin, Bu Benim Kodlayıcım** <:dragon:>")
    ...
    if (LastSee !== null && TimeOut - (Date.now() - LastSee) > 0) {
      ...
      if (message.content.length > 1) {
        ...
        const RevengeNYKS = new Discord.RichEmbed()
          ...
          .setDescription(
            " Bir Premium Üye Geldi,Hoşgeldin <@" + message.author.id + ">"
          )
          .setColor("RANDOM");
        message.channel
          .send(RevengeNYKS)
...
client.on("message", async message => {
  ...
  client.channels.get("703146826601201665").send(
